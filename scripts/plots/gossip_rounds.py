...
def gossip_round_comparison(files):
    dfs = []
    for file in files:
        x = pd.read_csv(
            file,
...
def gossip_energy_until_convergence(files):
    dfs = []
    for file in files:
        x = pd.read_csv(
            file,
    ...
    df.columns = pd.MultiIndex.from_product(
        [
            ["Fanout of 2 Nodes", "Fanout of 3 nodes", "Fanout of 4 nodes"],
            ["2 N/M", "3 N/M", "4 N/M"],
        ]
    )
...
def gossip_rounds(files):
    ...
    plt.tight_layout()
    plt.savefig("gossip_rounds.pdf", bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    gossip_rounds(files)
    table = gossip_round_comparison(files)
    gossip_energy_until_convergence(files)
